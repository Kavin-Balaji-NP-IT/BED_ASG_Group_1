...
console.log("Selected Dates:", localStorage.getItem("selectedDate"));
console.log("Selected IDs:", localStorage.getItem("selectedMedicationIds"));
...
function audioTrackSelections() {
  ...
  if (!audioListDiv) {
    console.error("Audio list div not found");
    return;
  }
...
// Select ringtone button activated and post into database
async function addSelectedRingtone(id, audioLink) {
  try {
    ...
    const response = await fetch(
      `http://localhost:3000/medications/${id}/ringtone`,
      {
        method: "PUT",
        headers,
